...
| **Type-MoE（100 步）** | **0.360281** | **0.389527** |
| 差值 | +0.0026 | +0.0079 |
| 相对差异 | +0.73% | +2.07% |

仅经过 100 步的最小训练，性能下降幅度仅为 0.73%（MSE），低于冷启动引入随机专家的预期扰动量级（通常 5—20%）。这一结果验证了分阶段冻结与零初始化残差两项保护机制的有效性：在新专家尚未充分预热时，预训练知识基本得到保留。

训练上下文窗口（128）与评估上下文窗口（512）存在 4× 的不匹配，若训练时对齐上下文长度，预计评估性能还有一定改善空间。
